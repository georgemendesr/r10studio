...
import { PlusIcon, TrashIcon, UploadIcon, PlayIcon, KeyboardIcon, EyeIcon } from "lucide-react";
...
const VideoSlidePage = () => {
  ...
  a.download = `${title.replace(/[^a-zA-Z0-9\s]/g, '_').trim() || 'video'}.${fileExtension}`;
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        // Pré-calcular quebra de linhas (performance) - FONTE 20% MAIOR
        let textLines: string[] = [];
        let totalTextLength = 0;
        if (captionText) {
          ctx.font = '800 67px Poppins, Arial, sans-serif'; // Fonte 20% maior (56px -> 67px)
          const maxW = canvas.width - 100 - 100; // Margens esquerda e direita de 100px cada
          ...
          if (current.trim()) textLines.push(current.trimEnd());
          // Calcular total de caracteres para animação proporcional
          totalTextLength = textLines.join(' ').length;
        }
        ...
        for (let frame = 0; frame < totalFrames; frame++) {
          ...
          // Determinar escala com CORTES SECOS (PUNCH ZOOM) - SEM interpolação
          let scale = 1.0;
          ...
          ctx.drawImage(img, dx, dy, drawW, drawH);          // Fade otimizado: usar imagem pré-carregada
          if (useFade && i > 0 && frame < 10) {
            const prevImg = preloadedImages[i - 1];
            ctx.globalAlpha = (10 - frame) / 10;
            // Aplicar mesma lógica de escala/alinhamento da imagem anterior
          ...
          // Flash otimizado nos primeiros 3 frames (não 5)
          if (useFlash && frame < 3) {
            ctx.fillStyle = '#FFFFFF';
            ctx.globalAlpha = 0.8 - (frame * 0.25); // Fade out do flash
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            ctx.globalAlpha = 1.0;
          }
          
          // Desenhar texto pré-calculado - TEXTO 20% MAIOR E POSIÇÃO MAIS ALTA
          if (textLines.length > 0) {
            ctx.font = '800 67px Poppins, Arial, sans-serif'; // Aumentado de 56px para 67px (+20%)
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
            const left = 100; // Aumentado de 60 para 100 (mais margem esquerda)
            const rightMargin = 100; // Margem direita também 100px
            const lh = 86; // Line height aumentado de 72 para 86px (+20%)
            const padX = 29, padY = 19; // Padding interno aumentado (+20%)
            const totalH = textLines.length * lh + (textLines.length - 1) * 7; // Espaçamento entre linhas aumentado
            let y = canvas.height - 300 - totalH; // SUBIU: Margem inferior de 180 para 300px

            // Efeito typewriter MAIS LENTO e natural - baseado na duração do slide
            const typewriterDurationMs = Math.max(2000, durationSec * 1000 * 0.4); // 40% da duração do slide, mínimo 2s
            const typewriterFrames = Math.floor((typewriterDurationMs / 1000) * FRAME_RATE); // Frames baseados na duração
            const progress = Math.min(1, frame / typewriterFrames);

            // Calcular quantos caracteres totais devem aparecer
            const totalCharsToShow = Math.floor(totalTextLength * progress);
            let charsShown = 0;

            for (let lineIndex = 0; lineIndex < textLines.length; lineIndex++) {
              const line = textLines[lineIndex];
              const maxW = canvas.width - left - rightMargin; // Usa margem direita
              const fullWidth = Math.min(maxW, ctx.measureText(line).width);
              
              // Determinar quantos caracteres desta linha devem aparecer
              let renderText = '';
              if (charsShown < totalCharsToShow) {
                const charsInThisLine = Math.min(line.length, totalCharsToShow - charsShown);
                renderText = line.slice(0, charsInThisLine);
                charsShown += line.length; // Conta todos os caracteres da linha (incluindo espaços)
              } else if (charsShown >= totalCharsToShow) {
                // Esta linha ainda não deve aparecer
                break;
              }
              ...
              if (renderText) {
                ...
                // Fundo vermelho exato - só aparece se há texto
                ctx.fillStyle = '#ef4444';
                ctx.fillRect(left, y + (lh - 56) / 2 - padY / 2, Math.min(fullWidth, textWidth) + padX * 2, 56 + padY); // Ajustado para fonte 56px
              ...
              y += lh + 6; // Espaçamento entre linhas aumentado
            }
          }

          // Desenhar watermark otimizado (se existir)
          if (watermarkImg) {
            const margin = 40;
          ...
          // Remover o requestAnimationFrame que estava causando delay
          // MediaRecorder já controla o timing dos frames automaticamente
        }
      }
      
      // Calcular duração total do vídeo
      const totalVideoMs = slides.reduce((acc, slide) => acc + (slide.durationSec || 5) * 1000, 0);
      const endingDurationMs = useEndingVideo && endingVideoUrl ? 3000 : 0; // 3 segundos para vinheta
